...
const SYSTEM_PROMPT = `You are MediAssist, a helpful and empathetic AI assistant for a hospital finder platform.
...
You can help with: finding hospitals by location or speciality, understanding medical terms, general health guidance, appointment booking info, and emergency contacts.
...
export default function MediAssistChatbot() {
  ...
  const sendMessage = async (text) => {
    ...
    const next = [...messages, { role: "user", content: msg, id: Date.now() }];
    setMessages(next);
    setInput("");
    setLoading(true);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "Connection error. Please try again.", id: Date.now() },
      ]);
    } finally {
      setLoading(false);
    }
    ...
    <>
      <style>{`
        ...
        .pulse-ring::before {
          ...
          background:#2ec4a0; animation:pulseRing 2s ease-out infinite;
        }
      ...
      <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end gap-3 font-sans">
        ...
        {open && !minimized && (
          <div
            className="chat-up flex flex-col rounded-3xl shadow-2xl overflow-hidden border border-[#daeaea]"
          ...
          >
            ...
            <div className="bg-[#0F5C5C] px-5 py-3.5 flex items-center gap-3 flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[#2ec4a0] text-[11px] mt-0.5 flex items-center gap-1">
                  <CrosshairIcon /> AI Health Assistant · Online
                </p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto chat-scroll px-4 py-4 space-y-3" style={{ background: "#f6fafa" }}>
                ...
                <div key={m.id} className={`msg-in flex gap-2 ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  <div
                    className={`max-w-[78%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                      m.role === "user"
                        ? "bg-[#0F5C5C] text-white rounded-tr-sm"
                        : "bg-white text-[#1a3333] rounded-tl-sm shadow-sm border border-[#e4f0f0]"
                    }`}
              ...
              {loading && (
                <div className="msg-in flex gap-2 justify-start">
                  <Avatar />
                  <div className="bg-white rounded-2xl rounded-tl-sm shadow-sm border border-[#e4f0f0]">
                    <TypingDots />
                  </div>
                </div>
              )}
              ...
              <div className="px-4 py-2.5 flex flex-wrap gap-1.5 border-t border-[#eaf4f4]" style={{ background: "#f6fafa" }}>
                {QUICK_REPLIES.map((q) => (
                  <button
                    ...
                    className="text-[11px] font-medium bg-white border border-[#0F5C5C]/25 text-[#0F5C5C] px-3 py-1.5 rounded-full hover:bg-[#0F5C5C] hover:text-white transition-all"
                  >
                    {q}
                  </button>
                ))}
            ...
            <div className="px-3 py-3 bg-white border-t border-[#eaf4f4] flex items-end gap-2 flex-shrink-0">
              <textarea
                ...
                rows={1}
                style={{ lineHeight: 1.5, maxHeight: 80, resize: "none" }}
            ...
            <p className="text-center text-[10px] text-gray-400 py-1.5 bg-white leading-none">
        ...
        {!open && (
          ...
          >
            <BotIcon size={22} />
            {unread && (
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#2ec4a0] rounded-full border-2 border-white text-[9px] font-bold flex items-center justify-center text-white">
                !
              </span>
            )}
          </button>
