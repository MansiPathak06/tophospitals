...
const HospitalCard = ({ hospital }) => (
  <div className="group bg-white rounded-2xl border border-[#e6f4f4] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden">
    <div className="relative w-full h-44 bg-[#e6f4f4] flex-shrink-0 overflow-hidden">
      ...
      {hospital.verified && (
        <span className="absolute top-2.5 left-2.5 bg-[#0F5C5C] text-white text-[10px] font-bold px-3 py-1 rounded-full">✓ Verified</span>
      )}
      {hospital.tag && (
        <span className="absolute top-2.5 right-2.5 bg-amber-400 text-amber-900 text-[10px] font-bold px-3 py-1 rounded-full">★ {hospital.tag}</span>
      )}
      {hospital.emergency && (
        <span className="absolute bottom-2.5 right-2.5 bg-red-500 text-white text-[10px] font-bold px-3 py-1 rounded-full">24/7 Emergency</span>
      )}
    </div>
    <div className="flex flex-col flex-1 p-4">
      ...
      <div className="flex gap-1.5 items-start mb-1.5">
        <span className="text-[#0F5C5C] text-[13px]">📍</span>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {hospital.specialities.slice(0, 3).map((s) => (
          <span key={s} className="bg-[#e6f4f4] text-[#0F5C5C] text-[11px] font-semibold px-2.5 py-1 rounded-full">{s}</span>
        ))}
        {hospital.specialities.length > 3 && (
          <span className="bg-gray-100 text-gray-400 text-[11px] px-2.5 py-1 rounded-full">+{hospital.specialities.length - 3} more</span>
        )}
      </div>
...
export default function HospitalsPage() {
  ...
  const handleShowLess = () => {
    document.getElementById("hospitals-top")?.scrollIntoView({ behavior: "smooth" });
      ...
      <div className="min-h-screen bg-[#f4fafa]">
        <div className="bg-white  border-b-2 border-[#e6f4f4] px-6 py-5">
          <div id="hospitals-top" className="max-w-[1280px] mx-auto flex items-end justify-between flex-wrap gap-4">
            <div>
              ...
              <h1 className="text-[#0F5C5C] text-3xl md:text-4xl font-bold mb-1">Top Hospitals Near You</h1>
              <p className="text-[#0F5C5C]/50 text-sm">{hospitals.length} hospitals listed · Moradabad, UP</p>
            </div>
            {hospitals.length > INITIAL_COUNT && !showAll && (
        ...
        <div className="max-w-[1280px] mx-auto px-6 py-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {visibleHospitals.map((h, index) => (
              <HospitalCard key={`${h.id}-${index}`} hospital={h} />
            ))}
          </div>
          {showAll && (
